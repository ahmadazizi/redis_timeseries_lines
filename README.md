...
Each timeseries is referred to as a `line` of data; so RedisTimeseriesLines maintains several lines of timeseries together. As a result, multiple timeseries values can be refered with a timestamp as if they are stored in a table with a timestamp and multiple columns.

Additionally RedisTimeseriesLines supports two levels of classifiers(c1, c2) to interact with data, plus support for timeframes and data compression(downsampling).

## About Redis
Redis is an open source (BSD licensed), in-memory data structure store used as a database, cache, message broker, and streaming engine.

## About RedisTimeSeries
RedisTimeSeries is a Redis module that adds a time series data structure to Redis. `RedisTimeseriesLines` uses RedisTimeSeries to store timeseries data.

## Installation

To install RedisTimeseriesLines, run the following command:

```pip install --upgrade redis_timeseries_lines```
...
To get started, simply create a class that inherits from `RedisTimeseriesLines`. Then set the properties `_name`, `_lines`, and `_timeframes`.

```python
    class Test(RedisTimeseriesLines):
        _name = 'test'
        _lines = ['l1', 'l2']
        _timeframes = {
...
class SensorData(RedisTimeseriesLines):
    _name = 'sensors'
    _lines = ['temp', 'hum']
    _timeframes = {
